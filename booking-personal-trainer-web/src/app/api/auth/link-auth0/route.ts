...
/**
 * Uses the Auth0 session to obtain a JWT, then calls Nest `/auth/link-auth0` with `{ token, password }`.
...
export const POST = async (req: NextRequest): Promise<NextResponse> => {
  ...
  const body: LinkAuth0Body = await req.json().catch(() => ({}));
  ...
  const apiRes = await fetch(`${apiUrl}/api/v1/auth/link-auth0`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ token: auth0Token, password }),
  });
